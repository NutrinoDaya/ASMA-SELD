...
import sys
import os
# Add parent directory to path to import main utils.py
parent_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, parent_dir)
import utils as parent_utils
least_distance_between_gt_pred = parent_utils.least_distance_between_gt_pred
jackknife_estimation = parent_utils.jackknife_estimation
load_labels = parent_utils.load_labels
organize_labels = parent_utils.organize_labels
import os
import warnings
...
def _process_single_file(args):
    """
    Process a single prediction file against its reference.
    Thread-safe function for multiprocessing.
    
    :param args: Tuple containing (pred_file, pred_files_path, ref_labels_entry, 
                 doa_thresh, dist_thresh, reldist_thresh, req_onscreen, nb_classes, average)
    :return: Tuple of metrics arrays for aggregation
    """
    (pred_file, pred_files_path, ref_labels_entry, 
     doa_thresh, dist_thresh, reldist_thresh, req_onscreen, nb_classes, average) = args
    
    try:
        # Create a local SELDMetrics instance for this file
        local_eval = SELDMetrics(
            doa_threshold=doa_thresh, 
            req_onscreen=req_onscreen,
            dist_threshold=dist_thresh, 
            reldist_threshold=reldist_thresh,
        ...
        nb_ref_frames = ref_labels_entry[1]
        pred_labels = organize_labels(pred_dict, max(nb_pred_frames, nb_ref_frames))
        
        # Calculate scores for this file
        local_eval.update_seld_scores(pred_labels, ref_labels_entry[0])
        
        # Return the accumulated metrics from this file
        return (pred_file, pred_labels if pred_labels else None, {
            'TP': local_eval._TP.copy(),
            'FP': local_eval._FP.copy(),
            'FP_spatial': local_eval._FP_spatial.copy(),
            'FN': local_eval._FN.copy(),
            'Nref': local_eval._Nref.copy(),
            'total_AngE': local_eval._total_AngE.copy(),
            'total_DistE': local_eval._total_DistE.copy(),
            'total_RelDistE': local_eval._total_RelDistE.copy(),
            'total_OnscreenCorrect': local_eval._total_OnscreenCorrect.copy(),
            'DE_TP': local_eval._DE_TP.copy(),
            'DE_FP': local_eval._DE_FP.copy(),
            'DE_FN': local_eval._DE_FN.copy()
        })
        
    except Exception as e:
        # Return error info for debugging
        return (pred_file, None, {'error': str(e)})
...
class ComputeSELDResults(object):
    ...
    def get_SELD_Results(self, pred_files_path, is_jackknife=False, use_multiprocessing=True, n_jobs=None):
        """
        ...
        :param use_multiprocessing: Whether to use multiprocessing for speed improvement.
        :param n_jobs: Number of parallel jobs. If None, uses cpu_count()-1.
        """
        # collect predicted files info
        pred_files = os.listdir(pred_files_path)
        
        if use_multiprocessing and len(pred_files) > 1:
            ...
            for pred_file in pred_files:
                if pred_file in self._ref_labels:
                    args = (
                        pred_file, 
                        pred_files_path, 
            ...
            try:
                ...
                for pred_file, pred_labels, metrics_dict in results:
                    if 'error' in metrics_dict:
                        print(f"Warning: Error processing {pred_file}: {metrics_dict['error']}")
                        continue
                    
                    # Aggregate metrics
                    eval._TP += metrics_dict['TP']
                    eval._FP += metrics_dict['FP'] 
                    eval._FP_spatial += metrics_dict['FP_spatial']
                    eval._FN += metrics_dict['FN']
                    eval._Nref += metrics_dict['Nref']
                    eval._total_AngE += metrics_dict['total_AngE']
                    eval._total_DistE += metrics_dict['total_DistE']
                    eval._total_RelDistE += metrics_dict['total_RelDistE']
                    eval._total_OnscreenCorrect += metrics_dict['total_OnscreenCorrect']
                    eval._DE_TP += metrics_dict['DE_TP']
                    eval._DE_FP += metrics_dict['DE_FP']
                    eval._DE_FN += metrics_dict['DE_FN']
                    
                    if is_jackknife and pred_labels is not None:
                        pred_labels_dict[pred_file] = pred_labels
                        
            except Exception as e:
                print(f"Multiprocessing failed, falling back to sequential processing: {e}")
                use_multiprocessing = False
        
        if not use_multiprocessing or len(pred_files) <= 1:
            ...
            for pred_cnt, pred_file in enumerate(pred_files):
                if pred_file not in self._ref_labels:
                    continue
                # Load predicted output format file
                pred_dict = load_labels(os.path.join(pred_files_path, pred_file), convert_to_cartesian=False)
                nb_pred_frames = max(list(pred_dict.keys())) if len(pred_dict) > 0 else 0
        ...
        if is_jackknife:
            ...
            if use_multiprocessing and len(pred_files) > 2:
                # Parallel jackknife computation
                def _jackknife_worker(leave_file):
                    leave_one_out_list = [f for f in pred_files if f != leave_file and f in pred_labels_dict]
                    local_eval = SELDMetrics(doa_threshold=self._doa_thresh, req_onscreen=self._req_onscreen,
                                           dist_threshold=self._dist_thresh, reldist_threshold=self._reldist_thresh,
                                           nb_classes=self._nb_classes, average=self._average)
                    for pred_file in leave_one_out_list:
                        if pred_file in self._ref_labels:
                            local_eval.update_seld_scores(pred_labels_dict[pred_file], self._ref_labels[pred_file][0])
                    
                    F_j, AngE_j, DistE_j, RelDistE_j, OnscreenAq_j, classwise_results_j = local_eval.compute_seld_scores()
                    leave_one_out_est = [F_j, AngE_j, DistE_j, RelDistE_j, OnscreenAq_j]
                    if len(classwise_results_j):
                        leave_one_out_est.extend(classwise_results_j.reshape(-1).tolist())
                    return leave_one_out_est
                
                try:
                    with Pool(processes=n_jobs) as pool:
                        partial_estimates = pool.map(_jackknife_worker, pred_files)
                except Exception as e:
                    print(f"Parallel jackknife failed, falling back to sequential: {e}")
                    use_multiprocessing = False
            
            if not use_multiprocessing:
                # Sequential jackknife computation
                for leave_file in pred_files:
                    leave_one_out_list = pred_files[:]
                    leave_one_out_list.remove(leave_file)
                    eval_jack = SELDMetrics(doa_threshold=self._doa_thresh, req_onscreen=self._req_onscreen,
                                           dist_threshold=self._dist_thresh, reldist_threshold=self._reldist_thresh,
                                           nb_classes=self._nb_classes, average=self._average)
                    for pred_cnt, pred_file in enumerate(leave_one_out_list):
                        if pred_file in pred_labels_dict and pred_file in self._ref_labels:
                            eval_jack.update_seld_scores(pred_labels_dict[pred_file], self._ref_labels[pred_file][0])
                    F_j, AngE_j, DistE_j, RelDistE_j, OnscreenAq_j, classwise_results_j = eval_jack.compute_seld_scores()
                    leave_one_out_est = [F_j, AngE_j, DistE_j, RelDistE_j, OnscreenAq_j]
                    if len(classwise_results_j):
                        leave_one_out_est.extend(classwise_results_j.reshape(-1).tolist())
