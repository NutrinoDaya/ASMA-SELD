...
class SELDModel(nn.Module):
    ...
    def calculate_flops(self, audio_input, video_input=None):
        ...
        # For accurate results, use tools like torchprofile or fvcore
        total_flops = 0
        
        # Estimate conv layer FLOPs
        # Each conv layer: output_h * output_w * kernel_h * kernel_w * input_channels * output_channels
        # This is a rough approximation
        
        # Audio input: [batch, 4, 250, 64] -> after conv blocks
        conv_flops = 0
        input_h, input_w = 250, 64
        input_channels = 4
        ...
        # RNN FLOPs (approximate)
        rnn_hidden = self.params['rnn_size']
        seq_len = self.params['label_sequence_length']
        rnn_flops = seq_len * rnn_hidden * rnn_hidden * 4 * self.params['nb_rnn_layers']  # 4 for LSTM gates
        
        # Attention FLOPs (approximate)
        attn_flops = seq_len * seq_len * rnn_hidden * self.params['nb_self_attn_layers']
        
        total_flops = conv_flops + rnn_flops + attn_flops
        
        return total_flops

    def get_model_stats(self):
        """Get model statistics including parameter count and estimated FLOPs."""
        total_params = sum(p.numel() for p in self.parameters())
        trainable_params = sum(p.numel() for p in self.parameters() if p.requires_grad)
        
        # Create dummy input for FLOP calculation
        dummy_audio = torch.randn(1, 4, 250, 64)
        dummy_video = torch.randn(1, 50, 49) if self.params.get('modality') == 'audio_visual' else None
