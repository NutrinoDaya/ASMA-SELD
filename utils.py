...
import os
# from torch.utils.tensorboard import SummaryWriter  # Commented to avoid TensorBoard issues
import time
import pickle
import librosa
...
def setup(params):
    ...
    # summary_writer = SummaryWriter(log_dir=str(log_dir))  # Commented to avoid TensorBoard issues
    summary_writer = None  # Using None instead of SummaryWriter to avoid TensorBoard issues
...
def load_video(video_file, fps):
    ...
    # Ensure we got at least some frames
    if len(pil_frames) == 0:
...
def extract_resnet_features(video_frames, resnet_preprocessor, resnet_backbone, device, batch_size=64):
    """
    Extracts ResNet-50 features from video frames with batched processing for speed.
    Args:
        ...
        batch_size (int): Number of frames to process in parallel on GPU (default: 64 for better GPU utilization).
    Returns:
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(preprocessed_images), batch_size):
            ...
            if next(resnet_backbone.parameters()).dtype == torch.float16:
                batch_tensor = batch_tensor.half()
            
            batch_features = resnet_backbone(batch_tensor)
            batch_features = torch.mean(batch_features, dim=1)
            all_features.append(batch_features.cpu().float())  # Convert back to FP32 for saving
        
        # Concatenate all batches
        vid_features = torch.cat(all_features, dim=0)
        return vid_features
